...
pub type SessionSupportCallback = Box<'static + FnOnce(Result<(), Error>) + Send>;

pub type SessionRequestCallback = Box<'static + FnOnce(Result<Session, Error>) + Send>;

impl Registry {
    pub fn new(xr: SessionBuilder) -> Registry {
            ...
            match msg {
                ...
                RegistryMsg::RequestSession(mode, callback) => {
                    for discovery in &mut self.discoveries {
                        if let Ok(session) = discovery.request_session(mode, self.xr.clone()) {
